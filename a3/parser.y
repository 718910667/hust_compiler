...
%token PLUS MINUS STAR DIV ASSIGNOP AND OR NOT IF ELSE WHILE RETURN INC DEC FOR LB RB COMP_PLUS COMP_MINUS
...
Exp:    Exp ASSIGNOP Exp {$$=mknode(ASSIGNOP,$1,$3,NULL,yylineno);strcpy($$->type_id,"ASSIGNOP");}//$$结点type_id空置未用，正好存放运算符
	  ...
	  | VarDec INC  {$$=mknode(INC,$1,NULL,NULL,yylineno);strcpy($$->type_id,"INC");}
      | VarDec DEC {$$=mknode(MMINUS,$1,NULL,NULL,yylineno);strcpy($$->type_id,"MMINUS");}
      | INC VarDec  {$$=mknode(DEC,$2,NULL,NULL,yylineno);strcpy($$->type_id,"DEC");}
      | DEC VarDec  {$$=mknode(UMMINUS,$2,NULL,NULL,yylineno);strcpy($$->type_id,"UMMINUS");}
	  | Exp	ID		{$$=mknode(ID,NULL,NULL,NULL,yylineno);strcpy($$->type_id,"1");}   //ID结点，标识符符号串存放结点的type_id
	  |VarDec LB INT RB {$$=mknode(ARRAY_DEF,$1,NULL,NULL,yylineno);$$->array_size[0]=$3;}//一维数组
      ;
